# Hello, I'm Eva 👋
Microsoft 365 | Identity Governance | Automation | Security-Focused Solutions

This page contains selected projects and technical write-ups from my work in Microsoft 365, automation, and security.

I'm a Microsoft 365 professional with hands-on experience designing and supporting solutions across **SharePoint Online, Power Apps, and Power Automate**. I work with **Entra ID**, MFA, and Conditional Access to strengthen identity governance, and I manage third-party application permissions across Microsoft 365 environments.

My work focuses on building structured workflows, maintaining clear technical documentation, and troubleshooting across M365 services. I also bring **5+ years of business ownership experience**, supporting operations across multi-site environments with strong coordination and process discipline.

### Cybersecurity Projects & Write-ups:
- [Elevating an eJPT Pivoting Exercise with ARP Scans and ProxyChains](https://www.linkedin.com/pulse/elevating-ejpt-pivoting-exercise-arp-scans-eva-rosalina-ananda-wbbnc/)
- [Simulating Real-World Persistence with Metasploit, BeEF, and Windows Registry](https://www.linkedin.com/pulse/simulating-real-world-persistence-metasploit-beef-windows-ananda-4b8ec/)
- [Security Automation Project: Real-Time HTTP Alerting with Python and Telegram](https://www.linkedin.com/pulse/security-automation-project-real-time-http-alerting-python-ananda-pvgac/)

### Microsoft 365 Governance & Automation Work:
- [User & Device Onboarding: Entra ID](https://github.com/evarananda/microsoft-365-governance-and-automation-projects/blob/main/1.%20Modern%20User%20%26%20Device%20Onboarding%20Framework.pdf)
- [Exchange Online Governance & Mail Flow Security: Exchange, Purview, Outlook](https://github.com/evarananda/microsoft-365-governance-and-automation-projects/blob/main/2.%20Exchange%20Online%20Governance%20%26%20Mail%20Flow%20Security.pdf)
- [Project Collaboration & Workflow Automation: Teams, SharePoint, Power Automate, Forms](https://github.com/evarananda/microsoft-365-governance-and-automation-projects/blob/main/3.%20Modern%20Project%20Collaboration%20%26%20Workflow%20Automation.pdf)
- [Bid Invitation Logging Automation with SharePoint and Power Automate](https://github.com/evarananda/microsoft-365-governance-and-automation-projects/blob/main/SharePoint%20Automation%20-%20Bid%20Invitation%20Logging%20(with%20sample%20data).jpg)
- [Daily Email Notification for Deleted Items in SharePoint](https://github.com/evarananda/microsoft-365-governance-and-automation-projects/blob/main/SharePoint%20Automation%20-%20Daily%20Email%20Notification%20for%20Deleted%20Items%20(with%20sample%20data).jpg)

### Business App Development:
- [Leave Management System with Power Apps](https://github.com/evarananda/leave-management-system-powerapps)
- [Helpdesk Ticketing System with Power Apps](https://github.com/evarananda/Helpdesk-Ticketing-System-PowerApps)
- [Random Question Picker with Power Apps](https://github.com/evarananda/random-question-picker-powerapps/tree/main)

### Active Directory Projects:
- [Active Directory Hands-On (Part 1): Connecting a Client to the Domain Controller, Managing Users & Groups, and Understanding File Permissions](https://www.linkedin.com/pulse/active-directory-hands-on-part-1-connecting-client-domain-ananda-fq9gc/)
- [Active Directory Hands-On (Part 2): File Permissions and GPOs](https://www.linkedin.com/pulse/active-directory-hands-on-part-2-file-permissions-gpos-ananda-ezkse/)

### Data Visualization
- [Unicorn Data (2023) - Web Scraping Python Code](https://github.com/evarananda/unicorn-webscraping)
- [Unicorn Data (2023) - Visualization in Tableau](https://public.tableau.com/app/profile/eva.ananda/viz/UnicornVisualizationNew/UnicornDashboard)

---

### 🎓 Certifications
- [CompTIA **Security+**](https://www.credly.com/badges/3fca09f4-d840-46ba-ad97-ba9b287a5299)
- [AWS **Cloud Practitioner**](https://www.credly.com/badges/5059bc04-5276-4194-b72c-0e172854d49b) 
- [eLearnSecurity **Junior Penetration Tester (eJPT)**](https://certs.ine.com/f4e9a15e-4a1e-4b78-8bdb-84a4bcc07e7b#acc.5oUNn8gz)

---

### 📫 Connect
- LinkedIn: https://www.linkedin.com/in/evarosalinaananda/
- Email: evarosalinaananda@gmail.com
- GitHub Projects: see my repositories

---
